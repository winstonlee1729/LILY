...
! Declaration of variables
    ...
    ! Flow field
    real*8, allocatable :: u(:,:), ustar(:,:), uold(:,:), uhat(:,:), du(:,:)
    real*8, allocatable :: v(:,:), vstar(:,:), vold(:,:), vhat(:,:), dv(:,:)
...
! SIMPLER algorithm starts here
    do iter = 1, maxiter
        ...
        ! Reapplying boundary conditions for uhat and vhat
        ! Inlet and outlet
        do j = 0, ny+1  
            uhat(1, j) = U_inf                  ! Inlet
            vhat(0, j) = 0.0D0                  ! v = 0 at inlet
            vhat(nx+1, j) = vhat(nx, j)         ! Zero gradient v at outlet
        enddo
        ...
        ! Top and bottom
        do i = 0, nx+1
            vhat(i, 1) = 0.0D0                  ! Bottom wall v = 0
            vhat(i, ny+1) = 0.0D0               ! Top wall v = 0
            uhat(i, 0) = -uhat(i, 1)            
            uhat(i, ny+1) = -uhat(i, ny)         
            ! If assuming flow is fully developed
            ! uhat(i, 0) = uhat(i, 1)             ! Bottom slip (zero shear)
            ! uhat(i, ny+1) = uhat(i, ny)         ! Top slip (zero shear)
        enddo
        ...
        ! Reapplying boundary conditions for ustar and vstar
        ! Inlet and outlet
        do j = 0, ny+1  
...
! Outputting solution
    open(unit=10, file='SS_Cylinder_SIMPLER.txt', status='unknown')
    do j = 1, ny
        do i = 1, nx
            xc = (dble(i) - 0.5D0) * dx
            yc = (dble(j) - 0.5D0) * dy
...
! Memory Deallocation
    deallocate(u, ustar, uold, uhat, du)
    deallocate(v, vstar, vold, vhat, dv)
    deallocate(p, pprime, bprime, b_p, temp_pprime, temp_p)

    stop
end program SS_Cylinder_SIMPLER