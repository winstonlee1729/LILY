...
! Declaration of variables
    ...
    parameter(nx = 300, ny = 120)
    parameter(maxiter = 100, max_step = 1000, output_freq = 5)

    ! For program convergence
    real*8, parameter :: tol = 1.0D-5
    real*8, parameter :: sor_tol = 1.0D-7
    ...
    ! SOR and temporal discretization
    ...
    ! Grain generation
    integer*4 :: ngrains, g, seed_size
    integer*4, allocatable :: seed(:)
    real*8 :: cx, cy, cr, dist, rnd1, rnd2, rnd3
...
! Initialization
    ...
    ! Generate random porous geometry (circular grains)
    is_fluid = 0
    ...
    seed = 19990426         ! In memory of CIH virus
    ...
    do g = 1, ngrains
        ...
        cx = (0.10D0 + 0.85D0 * rnd1) * Lx
        cy = rnd2 * Ly
        cr = (1.5D0 + 3.5D0 * rnd3) * dy
        do i = 1, nx
            do j = 1, ny
                dist = sqrt(((dble(i) - 0.5D0) * dx - cx)**2 + ((dble(j) - 0.5D0) * dy - cy)**2)
                if (dist < cr) then
                    is_fluid(i, j) = 0
                endif
            enddo
...
! Start time loop
    do time_step = 1, max_step
        ...
        do iter = 1, maxiter
            ...
            do i = 2, nx
                do j = 1, ny
                    if (is_fluid(i, j) == 1 .and. is_fluid(i-1, j) == 1) then
                        ...
                        ustar(i, j) = (w_node + e_node + n_node + s_node + p_past + (1.0D0 - alph_u) * ap * uold(i, j) + &
            ...
            do i = 1, nx
                do j = 2, ny
                    if (is_fluid(i, j) == 1 .and. is_fluid(i, j-1) == 1) then
                        ...
                        vstar(i, j) = (w_node + e_node + n_node + s_node + p_past + (1.0D0 - alph_v) * ap * vold(i, j) + &
            ...
            ! Correcting velocities and pressure
        ...
        write(*,'(A,I6,A,I4,A,ES12.4)') ' Step:', time_step, '  Inner iters:', iter, '  Max mass error:', bmax
        

        if (mod(time_step, output_freq) == 0) then
            write(vtk_filename, '(A, I6.6, A)') 'PORE_2D_', time_step, '.vtk'
            call write_vtk(trim(vtk_filename), u, v, p, is_fluid, nx, ny, dx, dy)
        endif
    enddo
...
! Deallocate memory
    ...
    stop
end program Porous_Media_SIMPLE

    
! This subroutine aims at outputting VTK files.
subroutine write_vtk(filename, u, v, p, is_fluid, nx, ny, dx, dy)
    implicit none
    ...
    write(20, '(A)') '2D porous-media flow (pore-scale resolved)'
    write(20, '(A)') 'ASCII'
    ...
    write(20, '(A)') 'SCALARS Solid float 1'
    write(20, '(A)') 'LOOKUP_TABLE default'
    do j = 1, ny
        write(20, '(5E15.6)') (dble(1 - is_fluid(i, j)), i = 1, nx)
    enddo
