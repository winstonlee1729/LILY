...
! Declaration of variables
    ...
    real*8 Pe_w, Pe_e, Pe_n, Pe_s, Pe_t, Pe_b
...
! Defining variables
    ...
    ab = Db * max(0.0D0, (1.0D0 - 0.1D0*abs(Pe_b))**5) + max(Fb, 0.0D0)
    at = Dt * max(0.0D0, (1.0D0 - 0.1D0*abs(Pe_t))**5) + max(-Ft, 0.0D0)
