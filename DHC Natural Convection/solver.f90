program DHC_Natural_Convection
    use omp_lib
    implicit none
...
! Declaration of variables
    ...
    parameter(maxiter = 100, max_step = 5000, output_freq = 20)
    ...
    parameter(T_hot = 1.0D0, T_cold = 0.0D0, T_ref = 0.5D0)
...
! Initialization
    ...
    ! Initialization of temperature field with a linear profile between hot and cold walls
    do j = 0, ny+1
        do i = 0, nx+1
            T(i, j) = T_hot - (T_hot - T_cold) * (dble(i) - 0.5D0) / dble(nx)
    ...
    do j = 0, ny+1
        T(0, j) = 2.0D0 * T_hot - T(1, j)
        T(nx+1, j) = 2.0D0 * T_cold - T(nx, j)
    enddo
...
! Start time loop
    do time_step = 1, max_step
        ...
        do iter = 1, maxiter
            ...
            do j = 1, ny
                do i = 1, nx
                    ...
                    if (i > 1) u(i, j) = ustar(i, j) + duu(i, j) * (pprime(i-1, j) - pprime(i, j))
                    if (j > 1) v(i, j) = vstar(i, j) + dvv(i, j) * (pprime(i, j-1) - pprime(i, j))
                enddo
            enddo
            !$omp end parallel do
        ...
        if (mod(time_step, output_freq) == 0) then
            write(vtk_filename, '(A, I6.6, A)') 'DHC_2D_', time_step, '.vtk'
            call write_vtk_dhc(trim(vtk_filename), u, v, p, T, nx, ny, dx, dy)
        endif
...
! Deallocate memory
    ...
    stop
end program DHC_Natural_Convection

    
    
    
! This subroutine aims at writing VTK output files.
subroutine write_vtk_dhc(filename, u, v, p, T, nx, ny, dx, dy)
    implicit none
    ...
    write(20, '(A)') '2D Differentially Heated Cavity (QUICK)'
    write(20, '(A)') 'ASCII'
