...
fortran_output_freq = 25;
total_steps = 2000; 

% Load the raw data from Fortran
disp('Loading data from phi_out.txt...');
if exist('phi_out.txt', 'file') ~= 2
    error('File phi_out.txt not found. Make sure it is in the same folder.');
end
rawData = load('phi_out.txt');
...
f = figure('Name', '2D Transient Convection-Diffusion without Source', 'Position', [100, 100, 600, 500]);
ha = axes(f);
...
for k = 1:length(target_frames)
    ...
    if end_row > size(rawData, 1)
        fprintf('\nWarning: Data for step %d (frame %d) was not found in phi_out.txt.\n', current_step, current_frame);
    ...
    % Save figure
    image_name = sprintf('phi_step_%04d.png', current_step);
    saveas(f, image_name);
