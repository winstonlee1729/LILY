      program Unsteady_Heat_Diffusion
      use omp_lib
      implicit none
      ...
      ap = ap_o + (aw + ae + an + as) * 0.5D0
      ap_prime = ap_o - (ae + aw + an + as) * 0.5D0
...
!$omp+ s_node, p_past, rhs, lhs, err) reduction(max:maxres)
              do j = 2, ny-1
                  do i = 2, nx-1
                      e_node = ae * 0.5D0 * (T(i+1, j) + Told(i+1, j))
                      w_node = aw * 0.5D0 * (T(i-1, j) + Told(i-1, j))
                      n_node = an * 0.5D0 * (T(i, j+1) + Told(i, j+1))
                      s_node = as * 0.5D0 * (T(i, j-1) + Told(i, j-1))
