...
! Declaration of variables
    ...
    parameter(alph_p = 0.7D0, alph_u = 0.3D0, alph_v = 0.3D0)
...
! Outputting solution
    open(unit=10, file='SS_Cylinder_SIMPLE.txt', status='unknown')
    do j = 1, ny
        do i = 1, nx
            xc = (dble(i) - 0.5D0) * dx
            yc = (dble(j) - 0.5D0) * dy
...
! Memory Deallocation
    ...
    stop
end program SS_Cylinder_SIMPLE