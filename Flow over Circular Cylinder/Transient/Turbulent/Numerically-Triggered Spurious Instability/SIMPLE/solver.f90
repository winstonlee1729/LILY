program Transient_Cylinder_SIMPLE
    use omp_lib
    implicit none
...
! Declaration of variables
    ...
    ! Re = rho * U_inf * (2R) / mu 
    parameter(rho = 10.0D0, mu = 0.001D0, U_inf = 1.0D0)
...
! Open output file
    open(unit=10, file='cylinder_transient_SIMPLE.txt', status='replace')
...
! Start time loop
    do time_step = 1, max_step
        ...
        do iter = 1, maxiter
            ...
            ! Top and bottom (Assume flow is fully-developed.)
            do i = 0, nx+1
...
! Deallocate memory
    ...
    stop
end program Transient_Cylinder_SIMPLE