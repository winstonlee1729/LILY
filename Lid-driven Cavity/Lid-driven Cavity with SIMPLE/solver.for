      program SIMPLE_Lid_Driven_Cavity
      implicit none
      
      ! Declaration of variables
      integer*4 nx, ny, maxiter, i, j, k, iter
      integer*4 count1, count2, count_rate
      parameter (nx = 100, ny = 100)
      parameter (maxiter = 500000)
      
      real*8 tol, time_elapsed
      parameter (tol = 1.0D-6)
      
      real*8 rho, mu, ulid, Lx, Ly
      real*8 alph_p, alph_u, alph_v, dx, dy
      parameter (rho = 1.0D0, mu = 0.01D0, ulid = 5.0D0)
      parameter (Lx = 1.0D0, Ly = 1.0D0)
      parameter (alph_p = 0.3D0, alph_u = 0.7D0, alph_v = 0.7D0)
      
      real*8 u(0:nx+1, 0:ny+1), ustar(0:nx+1, 0:ny+1)
      real*8 v(0:nx+1, 0:ny+1), vstar(0:nx+1, 0:ny+1)
      real*8 uold(0:nx+1, 0:ny+1), vold(0:nx+1, 0:ny+1)
      real*8 p(0:nx+1, 0:ny+1), pstar(0:nx+1, 0:ny+1)
      real*8 pprime(0:nx+1, 0:ny+1), bprime(0:nx+1, 0:ny+1)
      real*8 du(0:nx+1, 0:ny+1), dv(0:nx+1, 0:ny+1)
      
      real*8 ae, aw, an, as, ap, delta_F
      real*8 Fe, Fw, Fn, Fs
      real*8 De, Dw, Dn, Ds, bmax
      real*8 xc, yc, uc, vc, pc

      dx = Lx / dble(nx)
      dy = Ly / dble(ny)

      ! Initialization
      u(i,j)     = 0.0D0
      v(i,j)     = 0.0D0
      p(i,j)     = 0.0D0
      ustar(i,j) = 0.0D0
      vstar(i,j) = 0.0D0
      pstar(i,j) = 0.0D0
     
      
      ! Applying boundary condition
      do i = 0, nx+1
         u(i, ny+1) = ulid
      enddo
      
      ! Start the clock
      call system_clock(count1, count_rate)
      
      ! SIMPLE Algorithm
      do iter = 1, maxiter
         
         ! Save old velocities for explicit under-relaxation
         do i = 0, nx+1
            do j = 0, ny+1
               uold(i,j) = u(i,j)
               vold(i,j) = v(i,j)
            enddo
         enddo
         
         ! u-momentum predictor (west surface u(i,j))
         De = mu * (dy / dx)
         Dw = De
         Dn = mu * (dx / dy)
         Ds = Dn
         
         ! Start i=2 to protect the left solid wall
         do i = 2, nx
            do j = 1, ny
               Fe = 0.5D0 * rho * dy * (u(i,j) + u(i+1,j))
               Fw = 0.5D0 * rho * dy * (u(i-1,j) + u(i,j))
               Fn = 0.5D0 * rho * dx * (v(i-1,j+1) + v(i,j+1))
               Fs = 0.5D0 * rho * dx * (v(i-1,j) + v(i,j))
               
               delta_F = Fe - Fw + Fn - Fs
               
               aw = Dw + max(Fw, 0.0D0)
               ae = De + max(-Fe, 0.0D0)
               as = Ds + max(Fs, 0.0D0)   
               an = Dn + max(-Fn, 0.0D0)  
               ap = aw + ae + as + an + delta_F
               
               du(i,j) = dy / ap
               
               ustar(i,j) = (aw * uold(i-1,j) + ae * uold(i+1,j) + 
     +                       as * uold(i,j-1) + an * uold(i,j+1) + 
     +                       (pstar(i-1,j) - pstar(i,j)) * dy) / ap
            enddo
         enddo
         
         ! v-momentum predictor (south surface v(i,j))
         ! Start j=2 to protect the south (bottom) solid wall
         do i = 1, nx
            do j = 2, ny
               Fe = 0.5D0 * rho * dy * (u(i+1,j-1) + u(i+1,j))
               Fw = 0.5D0 * rho * dy * (u(i,j-1) + u(i,j))
               Fn = 0.5D0 * rho * dx * (v(i,j) + v(i,j+1))
               Fs = 0.5D0 * rho * dx * (v(i,j-1) + v(i,j))
               
               delta_F = Fe - Fw + Fn - Fs
               
               aw = Dw + max(Fw, 0.0D0)
               ae = De + max(-Fe, 0.0D0)
               as = Ds + max(Fs, 0.0D0)
               an = Dn + max(-Fn, 0.0D0)
               ap = aw + ae + as + an + delta_F
               
               dv(i,j) = dx / ap
               
               vstar(i,j) = (aw * vold(i-1,j) + ae * vold(i+1,j) + 
     +                       as * vold(i,j-1) + an * vold(i,j+1) + 
     +                       (pstar(i,j-1) - pstar(i,j)) * dx) / ap
            enddo
         enddo
         
         ! Rapplying boundary conditions for guessed velocity field
         do i = 0, nx+1
            ustar(i, 0)    = 0.0D0    
            ustar(i, ny+1) = ulid     
            vstar(i, 1)    = 0.0D0    
            vstar(i, ny+1) = 0.0D0    
         enddo
         do j = 0, ny+1
            ustar(1, j)    = 0.0D0    
            ustar(nx+1, j) = 0.0D0    
            vstar(0, j)    = 0.0D0
            vstar(nx+1, j) = 0.0D0
         enddo
         

         ! Calculate mass source generated by difference between acutal and guessed velocity field
         bmax = 0.0D0
         do i = 1, nx
            do j = 1, ny
               pprime(i,j) = 0.0D0
               bprime(i,j) = (rho * dy * ustar(i,j)) +
     +                       (rho * dx * vstar(i,j)) -
     +                       (rho * dy * ustar(i+1,j)) -
     +                       (rho * dx * vstar(i,j+1))
               
               if (abs(bprime(i,j)) > bmax) then
                  bmax = abs(bprime(i,j))
               endif
               
            enddo
         enddo
         
         ! Gauss-Seidel Iteration for pressure correction
         do k = 1, 100
            do i = 1, nx
               do j = 1, ny
                  aw = rho * dy * du(i,j)
                  ae = rho * dy * du(i+1,j)
                  as = rho * dx * dv(i,j)
                  an = rho * dx * dv(i,j+1)
                  
                  if (i == 1)  aw = 0.0D0
                  if (i == nx) ae = 0.0D0
                  if (j == 1)  as = 0.0D0
                  if (j == ny) an = 0.0D0
                  
                  ap = aw + ae + an + as
                  
                  pprime(i,j) = (ae * pprime(i+1,j) + 
     +                           aw * pprime(i-1,j) + 
     +                           an * pprime(i,j+1) + 
     +                           as * pprime(i,j-1) + 
     +                           bprime(i,j)) / ap
               enddo
            enddo
         enddo
         

         ! Field updates and explicit under-relaxation 

         do i = 1, nx
            do j = 1, ny
               p(i,j) = pstar(i,j) + alph_p * pprime(i,j)
               pstar(i,j) = p(i,j)
               
               if (i > 1) then
                  uc = ustar(i,j) + du(i,j) *                 
     +             (pprime(i-1,j) - pprime(i,j))
                  u(i,j) = alph_u * uc + (1.0D0 - alph_u) * uold(i,j)
               endif
               
               if (j > 1) then
                  vc = vstar(i,j) + dv(i,j) *                  
     +             (pprime(i,j-1) - pprime(i,j))
                  v(i,j) = alph_v * vc + (1.0D0 - alph_v) * vold(i,j)
               endif
               
            enddo
         enddo
         
         ! Check convergence
         !if (mod(iter, 100) .eq. 0) then
            write(*,*) 'Iteration: ', iter, '| Max mass error: ', bmax
         !endif
         
         if (bmax <= tol) exit
         
      enddo   ! SIMPLE algorithm stops here
      
      ! Stop the clock
      call system_clock(count2, count_rate)
      
      ! Calculate runtime
      time_elapsed = real(count2 - count1) / real(count_rate)

      write(*,*) '------------------------------------'
      write(*,*) 'Total Iterations: ', iter
      write(*,*) 'Total Time (Seconds): ', time_elapsed
      write(*,*) '------------------------------------'

      ! Outputting solution
      open(unit=10, file='cavity_SIMPLE.txt', status='unknown')
      do j = 1, ny
         do i = 1, nx
            xc = (dble(i) - 0.5D0) * dx
            yc = (dble(j) - 0.5D0) * dy
            
            uc = 0.5D0 * (u(i,j) + u(i+1,j))
            vc = 0.5D0 * (v(i,j) + v(i,j+1)) 
            pc = p(i,j)
            
            write(10, '(5E16.6)') xc, yc, uc, vc, pc
         enddo
      enddo
      close(10)

      stop
      end