...
! Declaration of variables
    ...
    parameter(alph_u = 0.3D0, alph_v = 0.3D0, alph_w = 0.3D0)
    ...
    real*8, allocatable :: w(:,:,:), wstar(:,:,:), wold(:,:,:), dww(:,:,:), wn(:,:,:), what(:,:,:)
    real*8, allocatable :: p(:,:,:), pprime(:,:,:), bprime(:,:,:), b_p(:,:,:)
    real*8, allocatable :: temp_pprime(:,:,:), p_temp(:,:,:)
...
! Allocate memory
    ...
    allocate(p(0:nx+1, 0:ny+1, 0:nz+1), pprime(0:nx+1, 0:ny+1, 0:nz+1), bprime(0:nx+1, 0:ny+1, 0:nz+1), b_p(0:nx+1, 0:ny+1, 0:nz+1)) 
    allocate(temp_pprime(0:nx+1, 0:ny+1, 0:nz+1), p_temp(0:nx+1, 0:ny+1, 0:nz+1))
            ...
            do i = 2, nx
                do j = 1, ny
                    do k = 1, nz
                        ...
                        uhat(i, j, k) = (w_node + e_node + n_node + s_node + b_node + t_node + p_past ) / ap
                    enddo
                enddo
            enddo
            !$omp end parallel do
            ...
            do i = 1, nx
                do j = 2, ny
                    do k = 1, nz
                        ...
                        vhat(i, j, k) = (w_node + e_node + n_node + s_node + t_node + b_node + p_past) / ap
                    enddo
                enddo
            enddo
            !$omp end parallel do
            ...
            do i = 1, nx
                do j = 1, ny
                    do k = 2, nz
                        ...
                        what(i, j, k) = (w_node + e_node + n_node + s_node + t_node + b_node + p_past) / ap
                    enddo
                enddo
            enddo
            !$omp end parallel do
         
            ! Reapplying boundary conditions for uhat, vhat, and what
            !$omp parallel do collapse(2)
            do i = 0, nx+1
                do k = 0, nz+1
                    uhat(i, 0, k)    = 0.0D0    
...
! Deallocate memory
    ...
    deallocate(p, b_p, pprime, bprime)
    deallocate(temp_pprime, p_temp)
