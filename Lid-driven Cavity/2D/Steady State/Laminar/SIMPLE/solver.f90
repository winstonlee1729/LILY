program SIMPLE_Lid_Driven_Cavity
    use omp_lib
    implicit none
...
! SIMPLE algorithm starts here
    ...
    enddo  ! SIMPLE algorithm stops here
...
! Outputting solution
    open(unit=10, file='cavity_SIMPLE.txt', status='unknown')
    do j = 1, ny
        do i = 1, nx
            xc = (dble(i) - 0.5D0) * dx
            yc = (dble(j) - 0.5D0) * dy
...
! Memory Deallocation
    ...
    stop
end program SIMPLE_Lid_Driven_Cavity