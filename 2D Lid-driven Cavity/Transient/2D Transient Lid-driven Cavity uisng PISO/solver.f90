...
! Initialization of flow field (t = 0)
    u = 0.0D0; v = 0.0D0; p = 0.0D0
    ustar = 0.0D0; vstar = 0.0D0; pstar = 0.0D0
    ustarstar = 0.0D0; vstarstar = 0.0D0; pstarstar = 0.0D0
    un = 0.0D0; vn = 0.0D0
...
! Start time loop
    do time_step = 1, max_step
        ...
        do iter = 1, maxiter
            ...
            ! Save old velocities and reset correction arrays
            !$omp parallel do collapse(2)
            do i = 0, nx+1
                do j = 0, ny+1
                    uold(i,j) = u(i,j)
