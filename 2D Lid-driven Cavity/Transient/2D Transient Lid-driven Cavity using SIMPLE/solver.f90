...
! Open output file
    open(unit=10, file='cavity_transient_SIMPLE.txt', status='replace')
        ...
        do iter = 1, maxiter
            ...
            do i = 2, nx
                do j = 1, ny
                    ...
                    ! Pressure gradient evaluated implicitly at the current iteration
            ...
            ! v-momentum predictor (Theta Scheme)
            !$omp parallel do private(j, Fe, Fw, Fn, Fs, delta_F, aw_s, ae_s, as_s, an_s, &
            !$omp ap_s, ap_prime, ap, w_node, e_node, s_node, n_node, p_past)
            do i = 1, nx
                do j = 2, ny
